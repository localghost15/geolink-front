...
const POSReceipt = ({ selectedServices, visitId }) => {
    ...
    const quantities = selectedServices.reduce((acc, service) => {
        acc[service.id] = (acc[service.id] || 0) + 1;
        return acc;
    }, {});

    // Ensure unique services with correct quantities
    const uniqueServices = selectedServices.map(service => ({
        ...service,
        count: quantities[service.id] || 0
    }));

    const handleSubmit = () => {
        const serviceCountArray = buildServiceCountArray();

        // Используем FormData для отправки данных в нужном формате
        const formData = new FormData();
        serviceCountArray.forEach(({id, count}) => {
            formData.append(`service[${id}]`, count.toString());
            formData.append(`service_count[${id}]`, count.toString());
        });
    ...
    return (
        <>
            ...
            <Dialog
                ...
                open={open}
                className="bg-transparent flex justify-center max-w-min shadow-none"
                handler={handleOpen}
            >
                                    ...
                                    <div className="md:mx-6 md:py-40">
                                        <Card className="shadow-none">
                                            <CardBody className="flex flex-col gap-4">
                                                <Typography variant="h4" color="blue-gray">Тўлов</Typography>
                                                <Typography className="-mb-2" variant="h7">Сони</Typography>
                                                <Input value={amount} onChange={(e) => setAmount(e.target.value)} label="Тўлов Миқдори" type="number" size="lg" />
                                                <Typography className="-mb-2" variant="h7">Тўлов қолгани</Typography>
                                                <Input label="Тўлов қолгани" value={remainingAmount.toFixed(2)} disabled type="number" size="lg" />
                                                <Typography className="-mb-2" variant="h7">Общая сумма</Typography>
                                                <Input label="Общая сумма" value={calculateTotal().toFixed(2)} disabled type="number" size="lg" />
                                            </CardBody>
                                            <CardFooter className="pt-0">
                                                <Button onClick={handleSubmit} className="flex items-center gap-x-1 justify-center">
                                                    <PrinterIcon className="w-4 h-4"/>Тўлаш ва чекни чикариш
                                                </Button>
                                            </CardFooter>
                                        </Card>
                                    </div>
                                </div>
                                <div className="flex items-center px-12 rounded-b-lg lg:w-6/12 lg:rounded-r-lg lg:rounded-bl-none login-bg">
                                    <Card id="invoice-POS">
                                        <center id="top">
                                        ...
                                        <div id="mid">
                                            <div className="info text-center">
                                                ...
                                                <p>Манзил : Бухоро, Мустакиллик 31 кучаси<br/>Phone : 555-555-5555</p>
                                            </div>
                                        </div>
                                        <div id="bot">
                                            <div id="table">
                                            ...
                                            <div id="legalcopy">
                                                <p className="legal">
                                                    <strong>Biznesingiz uchun rahmat!</strong>&nbsp;To'lov 31 kun ichida kutilmoqda; shu vaqt ichida ushbu hisob-fakturani qayta ishlang. Kechiktirilgan hisobvaraq-fakturalar uchun oyiga 5% foiz undiriladi.
                                                </p>
                                            </div>
