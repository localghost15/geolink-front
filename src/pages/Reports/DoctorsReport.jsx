...
import {EditOutlined, DeleteOutlined, SearchOutlined, FileExcelOutlined} from '@ant-design/icons';
import Highlighter from 'react-highlight-words';
import {Tooltip} from "@material-tailwind/react";
import {EyeIcon, PencilIcon, TrashIcon} from "@heroicons/react/24/solid";
import {getDoctorsReport, getPatientReport} from "../../services/reports/reportsService";
...
const DoctorsReport = () => {
    ...
    const handleShowReport = async () => {
        ...
        try {
            ...
            const apiResponse = await getDoctorsReport(start_at, end_at);
    ...
    const columns = [
        ...
        {
            title: 'Нечта бемор кўрди',
            dataIndex: 'visits_count',
            key: 'visits_count',
            sorter: (a, b) => new Date(a.visits_count) - new Date(b.visits_count),
            ...getColumnSearchProps('visits_count'),
        },
        {
            title: 'Сумма',
            dataIndex: 'phone',
        ...
        },
        {
            ...
            render: (_, record) => (
                ...
                <Tooltip className="border border-blue-gray-50 text-black bg-white px-4 py-3 shadow-xl shadow-black/10" content="Малумот">
                     <Link to={`/reports/doctors/${record.id}?start_at=${dateRange[0] ? dateRange[0].format('YYYY-MM-DD') : ''}&end_at=${dateRange[1] ? dateRange[1].format('YYYY-MM-DD') : ''}`}>
                            <Button icon={<EyeIcon className="h-4 w-4" />} type="dashed" className="rounded-full" size="sm" />
                        </Link>
                </Tooltip>
            </span>
            ),
    ...
    return (
        <div>
            <div className="px-10">
                <h1 className="text-xl font-semibold mb-3">Врачлар бўйича хисобот</h1>
                <div className="flex justify-between w-full gap-2">
                    ...
                    <Button type="default" className="items-center flex" icon={<RiFileExcel2Fill />} onClick={exportToExcel} disabled={reportData.length === 0}>
                        Экспорт в Excel
                    </Button>
                </div>
